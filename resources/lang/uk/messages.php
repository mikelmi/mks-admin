<?php

return [
    'title' => 'AdminPanel',
    'Hello' => 'Привіт',
    'Search' => 'Пошук',
    'Cancel' => 'Скасувати',
    'Add' => 'Додати',
    'Create' => 'Створити',
    'Edit' => 'Редагувати',
    'Save' => 'Зберегти',
    'Delete' => 'Видалити',
    'View Site' => 'Відкрити сайт',
    'Language' => 'Мова',
    'My Profile' => 'Мій профайл',
    'Toggle menu' => 'Показати/Приховати меню',
    'Delete Selected' => 'Видалити відмічені',
    'Save and Close' => 'Зберегти і закрити',
    'Save and New' => 'Зберегти і додати новий',
    'Created at' => 'Дата створення',
    'Status' => 'Статус',
    'Updated at' => 'Дата змін',
    'Activate' => 'Активувати',
    'Deactivate' => 'Деактивувати',
    'Active' => 'Активний',
    'Inactive' => 'Неактивний',
    'Trash' => 'Смітник',
    'Move to trash' => 'Перемістити в смітник',
    'Restore' => 'Відновити',
    'In Trash' => 'У смітнику',
    'Save and Restore' => 'Зберегти і відновити',
    'action_text' => 'Якщо у вас виникли проблеми з натисканням кнопки ":actionText", скопіюйте та вставте URL нижче в свій веб-браузер',
    'Refresh' => 'Оновити',
    'selected_count' => 'Відмічені',
    'Yes' => 'Так',
    'No' => 'Ні',
    'Saved' => 'Збережено',
    'confirm_delete' => 'Ви впевнені, що хочете видалити цей елемент?',
    'confirm_delete_selected' => 'Ви впевнені, що хочете видалити вибрані елемент(и)?',
];
